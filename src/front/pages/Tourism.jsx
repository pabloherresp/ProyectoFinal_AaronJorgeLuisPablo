...
return(
    ...
    <div className="pb-5 container bg-white my-5 rounded myActivityCard fontFamily">
        ...
        <div className="row justify-content-around">
           {returnAllTourismActivities()?.map((activity,i) => <Link key={i} className="text-decoration-none valor-card2 col-lg-4 col-md-6 col-sm-12 mt-4" to={'/activities/' + activity.id}><ActivityCard className="mw-100" img={activity.info_activity.media[0]} title={activity.info_activity.name} origin={activity.meeting_point} description={activity.info_activity.desc.slice(0,40)} timeleft={returnCounter(returnAllTourismActivities()[i]?.start_date)}></ActivityCard></Link>)}
        </div>
    </div>
