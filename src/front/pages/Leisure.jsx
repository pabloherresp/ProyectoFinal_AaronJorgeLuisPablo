...
export const Leisure = () => {
    ...
    return (
        ...
        <div className="pb-5 container px-5 bg-white my-5 rounded myActivityCard fontFamily">
            ...
            <div className="row justify-content-around">
                {store.all_activities?.filter((item) => item.info_activity.type == "LEISURE" && new Date(item.start_date) > new Date(Date.now() + 7200000)).sort((a, b) => new Date(a.start_date) - new Date(b.start_date)).slice(0, 3).map((item, i) => {
                    return <div key={i} className="col-lg-4 col-md-6 col-sm-12 mt-4">
            ...
            <div className="row justify-content-around">
                {returnAllLeisureActivities()?.map((activity, i) =>
