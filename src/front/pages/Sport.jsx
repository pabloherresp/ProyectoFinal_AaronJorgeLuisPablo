...
function returnAllSportActivities(){
     ...
     for(let i = 0; i<store?.all_activities.length;i++){
         ...
         if(store?.all_activities[i]?.info_activity?.type=="SPORT" && store?.all_activities[i]?.is_finished==false){
 ...
 function mostValuatedActivities(){
    ...
    return returnAllSportActivities().sort((a,b) => b.info_activity.rating - a.info_activity.rating).filter((item,index) => index < 3)
...
var fechaActual = new Date(fechaEnMiliseg)
    ...
    return returnAllSportActivities().sort((a,b) => new Date(a.start_date) - new Date(b.start_date)).filter((item,index) => new Date(item.start_date) > fechaActual).slice(0,3)
