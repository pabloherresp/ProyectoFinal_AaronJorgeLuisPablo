...
def create_user():
    ...
    try:
        user = Users(email=data["email"], password=generate_password_hash(data["password"]), username=data["username"], name=data["name"], surname=data["surname"], NID=data["NID"], telephone=data["telephone"], avatar_url=data["avatar_url"])
        db.session.add(user)
        db.session.commit()
...
def edit_user(id):
    ...
    if not user:
        return jsonify({"error": "User not found"}), 404
    client = db.session.execute(select(Clients).where(Clients.user_id == user.id)).scalar_one_or_none()
    prof = db.session.execute(select(Professionals).where(Professionals.user_id == user.id)).scalar_one_or_none()

    data = request.json
    if not client or "is_professional" in data and data["is_professional"] == "true" and not prof:
    ...
    user_cells = ["username", "email", "telephone", "avatar_url", "name", "surname", "NID"]
    client_cells = ["address", "city", "gender", "birthdate"]
...
def add_fav(id):
    ...
    fav = Favourites(client_id=id, info_activity_id=data["info_activity_id"])
    db.session.add(fav)
    try:
        db.session.commit()
    except Exception as e:
...
def delete_fav(id):
    ...
    data = request.json
    stmt = select(Favourites).where(and_(Favourites.client_id==id,Favourites.info_activity_id==data["info_activity_id"]))
